...
// Kiro CLI stop hook (standard) — full data flow matching main.go wiring.
// Store → embed + δ-mem + turbovec + self-model        (service layer)
// Learn → embed + δ-mem + turbogo + IBNN reinforce     (thoughts engine)
// Both called = all core layers touched.
...
func main() {
	...
	for _, f := range facts {
		// Store → turbovec (service layer: embed + δ-mem + turbovec + self-model)
		client.Store(ctx, &pb.StoreRequest{Owner: owner, Key: f.key, Content: f.content})
		// Learn → turbogo (thoughts engine: embed + δ-mem + turbogo + IBNN reinforce)
		client.Learn(ctx, &pb.LearnRequest{Owner: owner, Fact: f.content})
	}
}
