...
// Kiro CLI stop hook (deep) — uses StoreDeep RPC for full-pipeline storage in one call.
// StoreDeep = embed → δ-mem → turbovec → turbogo → IBNN reinforce → temporal → self-model → persist.
// Single RPC, all 12 layers.
