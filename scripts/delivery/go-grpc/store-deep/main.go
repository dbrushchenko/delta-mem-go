...
// Kiro CLI stop hook (deep) — uses Learn RPC to absorb facts through all layers.
// Learn goes through: embed → δ-mem → turbogo → IBNN reinforce → self-model.
// Heavier than Store but produces deeper memory integration.
...
func main() {
	...
	for _, fact := range facts {
		client.Learn(ctx, &pb.LearnRequest{Owner: owner, Fact: fact})
	}
}

func extractFacts(response string) []string {
	var facts []string
